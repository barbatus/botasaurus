...
from .utils import write_json as format_write_json
...
def _read_json_files(file_paths):
    ...
    results = Parallel(n_jobs=-1)(
        delayed(safe_get)(file_path) for file_path in file_paths
    )
    return results
...
def _delete_corrupted_cached_items(file_paths):
    ...
    results = Parallel(n_jobs=-1)(
        delayed(safe_corrupted_get)(file_path) for file_path in file_paths
    )
    return results
...
def _delete_items_by_filter(func, items, should_delete_item):
    ...
    if collected_honeypots:
        ...
        while True:
            result = input(
                f"Should we delete {len(collected_honeypots)} items in {path}? (Y/n): "
            )
...
class Cache:
    ...
    def get_items(func, items=None, max=None):
        ...
        fn_name = getfnname(func)
        paths = [
            relative_path(f"{Cache.cache_directory}{fn_name}/{r}.json") for r in hashes
        ]
