...
from botasaurus.cache import _delete_items, _has, _remove, read_json, write_json

from .errors import JsonHTTPResponseWithMessage
from .utils import path_task_results_cache, path_task_results_tasks
...
def _read_json_files_dict(file_paths):
    ...
    results = Parallel(n_jobs=-1)(
        delayed(
            lambda item: {
                "key": item,
                "result": _get(os.path.join(path_task_results_cache, item)),
            }
        )(file_path)
                    ...
                    try:
                        ...
                        for i, split_line in enumerate(split_lines):
                            try:
                                ...
                                print(
                                    f"Failed to parse line {line_number}, part: {split_line}"
                                )
