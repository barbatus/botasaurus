...
def open_browser():
    #   sleep(1)
    while not is_server_ready("http://localhost:3001/"):
        sleep(0.1)
...
def print_frontend_run_message():
    print("Starting frontend server at http://localhost:3001")
...
def run():
    if "--help" in sys.argv:
        ...
        if not Server.get_scrapers_names():
            raise RuntimeError(
                "No scrapers found. Please add a scraper using Server.add_scraper."
            )
